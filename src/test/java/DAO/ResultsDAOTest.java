...
public class ResultsDAOTest {
    ...
    public static void setup() {
        ...
        for (int i = 1; i <= QUIZZES_NUM; i++) {
            ...
            QUIZZES_ID[i - 1] = quizzesDAO.getQuizByName("QZ" + i).getQuizId();
        }
    ...
    public void testAddResult() {
        resultsDAO.addResult(USERS_ID[2], QUIZZES_ID[3], 10, 2000);
        ArrayList<Result> res = resultsDAO.getUserResultsOnQuiz(USERS_ID[2], QUIZZES_ID[3]);
        assertEquals(1, res.size());
        assertEquals(10, res.get(0).getScore());

        resultsDAO.addResult(USERS_ID[3], QUIZZES_ID[5], 30, 3000);
        res = resultsDAO.getUserResultsOnQuiz(USERS_ID[3], QUIZZES_ID[5]);
        assertEquals(1, res.size());
        assertEquals(30, res.get(0).getScore());

        resultsDAO.addResult(USERS_ID[4], QUIZZES_ID[0], 14, 500);
        res = resultsDAO.getUserResultsOnQuiz(USERS_ID[4], QUIZZES_ID[0]);
        assertEquals(1, res.size());
        assertEquals(14, res.get(0).getScore());

        resultsDAO.addResult(USERS_ID[0], QUIZZES_ID[0], 10, 1000);
        resultsDAO.addResult(USERS_ID[0], QUIZZES_ID[0], 15, 800);
        res = resultsDAO.getUserResultsOnQuiz(USERS_ID[0], QUIZZES_ID[0]);
    ...
    public void testGetQuizResults() {
        resultsDAO.addResult(USERS_ID[0], QUIZZES_ID[1], 40, 900);
        ArrayList<Result> res = resultsDAO.getQuizResults(QUIZZES_ID[1]);
        assertEquals(1, res.size());
        assertEquals(QUIZZES_ID[1], res.get(0).getQuizId());
        assertEquals(USERS_ID[0], res.get(0).getUserId());
        assertEquals(40, res.get(0).getScore());
        assertEquals(900, res.get(0).getSpentTime());

        resultsDAO.addResult(USERS_ID[3], QUIZZES_ID[5], 50, 120);
        res = resultsDAO.getQuizResults(QUIZZES_ID[5]);
        assertEquals(1, res.size());
        assertEquals(QUIZZES_ID[5], res.get(0).getQuizId());
    ...
    public void testGerQuizResultsHard() {
        for (int i = 0; i < USERS_NUM; i++) {
            resultsDAO.addResult(USERS_ID[i], QUIZZES_ID[0], 100 * (USERS_NUM - i), 1000 * (USERS_NUM - i));
        }

        ArrayList<Result> res = resultsDAO.getQuizResults(QUIZZES_ID[0]);
        assertEquals(USERS_NUM, res.size());
        for (int i = 0; i < USERS_NUM; i++) {
            assertEquals(QUIZZES_ID[0], res.get(i).getQuizId());
    ...
    public void testUserResults() {
         ...
         */
        resultsDAO.addResult(USERS_ID[0], QUIZZES_ID[5], 100, 1000);
        resultsDAO.addResult(USERS_ID[0], QUIZZES_ID[2], 100, 2000);
        resultsDAO.addResult(USERS_ID[0], QUIZZES_ID[3], 100, 3000);
    ...
    public void testFriendsResults(){
        ...
        resultsDAO.addResult(USERS_ID[1], QUIZZES_ID[0], 100, 1000);
        resultsDAO.addResult(USERS_ID[1], QUIZZES_ID[1], 300, 2000);
        resultsDAO.addResult(USERS_ID[2], QUIZZES_ID[1], 200, 3000);

        ArrayList<Result> res = resultsDAO.getUserFriendsResultOnQuiz(USERS_ID[0], QUIZZES_ID[0]);

        assertEquals(1, res.size());
        assertEquals(USERS_ID[1], res.get(0).getUserId());
        assertEquals(QUIZZES_ID[0], res.get(0).getQuizId());
        assertEquals(100, res.get(0).getScore());
        assertEquals(1000, res.get(0).getSpentTime());


        res = resultsDAO.getUserFriendsResultOnQuiz(USERS_ID[0], QUIZZES_ID[1]);
        ...
        assertEquals(QUIZZES_ID[1], res.get(1).getQuizId());
